...
func (runner *Runner) Run() error {
	...
	err = runner.copyApp(runner.config.AppDir(), filepath.Join(runner.config.OutputDropletDir(), "app"))
	if err != nil {
		return newDescriptiveError(err, "failed to copy compiled droplet")
	}

	err = os.MkdirAll(filepath.Join(runner.config.OutputDropletDir(), "tmp"), 0755)
	if err != nil {
		return newDescriptiveError(err, "failed to set up droplet filesystem")
	}

	err = os.MkdirAll(filepath.Join(runner.config.OutputDropletDir(), "logs"), 0755)
	if err != nil {
		return newDescriptiveError(err, "failed to set up droplet filesystem")
	}
...
func (runner *Runner) detect() (string, string, string, error) {
	for _, buildpack := range runner.config.BuildpackOrder() {
		...
		err = runner.run(exec.Command(filepath.Join(buildpackPath, "bin", "detect"), runner.config.AppDir()), output)
...
func (runner *Runner) compile(buildpackDir string) error {
	return runner.run(exec.Command(filepath.Join(buildpackDir, "bin", "compile"), runner.config.AppDir(), runner.config.BuildArtifactsCacheDir()), os.Stdout)
}

func (runner *Runner) release(buildpackDir string) (Release, error) {
	output := new(bytes.Buffer)

	err := runner.run(exec.Command(filepath.Join(buildpackDir, "bin", "release"), runner.config.AppDir()), output)
	if err != nil {
		return Release{}, err
	}
	...
	if err != nil {
		return Release{}, errors.New("buildpack's release output invalid")
	}
...
func (runner *Runner) copyApp(appDir, stageDir string) error {
	return runner.run(exec.Command("xcopy", "/S", "/I", appDir, stageDir), os.Stdout)
}
